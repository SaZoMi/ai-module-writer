...
// Usage: node json-to-module.js <json-file> [output-dir]
...
const supportedGames = data.supportedGames || ['all'];

// Pick the "latest" version, or the first one
let version = data.versions.find(v => v.tag === 'latest') || data.versions[0];
if (!version) {
...
// Write module.json
fs.writeFileSync(path.join(outputDir, 'module.json'), JSON.stringify({
  name,
  author,
...
// Write config.json
try {
  const configSchema = typeof version.configSchema === 'string'
...
try {
  const uiSchema = typeof version.uiSchema === 'string'
...
if (version.commands && version.commands.length > 0) {
  const cmdsDir = path.join(outputDir, 'commands');
  ...
  for (const cmd of version.commands) {
    ...
    fs.writeFileSync(path.join(cmdDir, 'command.json'), JSON.stringify({
      trigger: cmd.trigger,
...
if (version.hooks && version.hooks.length > 0) {
  ...
  for (const hook of version.hooks) {
    ...
    fs.writeFileSync(path.join(hookDir, 'hook.json'), JSON.stringify({
...
if (version.cronJobs && version.cronJobs.length > 0) {
  const cronDir = path.join(outputDir, 'cronjobs');
  ...
  for (const cron of version.cronJobs) {
    ...
    fs.writeFileSync(path.join(cjDir, 'cronjob.json'), JSON.stringify({
...
if (version.functions && version.functions.length > 0) {
  const fnDir = path.join(outputDir, 'functions');
...
if (version.permissions && version.permissions.length > 0) {
  fs.writeFileSync(path.join(outputDir, 'permissions.json'), JSON.stringify(version.permissions, null, 2) + '\n');
